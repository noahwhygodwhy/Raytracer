...
double Gp(dvec3 v, dvec3 n, dvec3 h, double a) {
	...
	/*double wdotm = glm::dot(v, h);
	double wdotm2 = glm::pow(glm::dot(v, h), 2);

	double left = Xchi(wdotm / glm::dot(v, n));
	double right = 1.0 + glm::sqrt(1 + ((a * a) * ((1.0 - wdotm2) / wdotm2)));
	return left * right;*/
}
//wi is the 
...
/*dvec3 CookTorance(
	...
	dvec3 wo = outgoingRay.inverseDirection;
	dvec3 wi = incomingRay.direction;



	dvec3 n = minRayResult.normal;
	double a = 1.0 - mat->getSmoothness(uv);

	double fresnel = 0.0;
	if (mat->getMetalness(uv) > 0.0) {
		fresnel = fConductor(mat->getNI(uv), wi, wo);
	} else {
		fresnel = fDialectric(prevIOR, mat->getNI(uv), wi, wo);
	}
	if(prd)printf("fresnel: %f\n", fresnel);
	fresnel = 1.0 - fresnel;

	kS = fresnel;


	dvec3 h = glm::normalize(wo + wi);

	double geometry = G(wi, wo, n, h, roughness);
	//geometry = 1.0 - geometry;

	if (prd)printf("geometry: %f\n", geometry);

	double cosT = glm::dot(wi, n);
	double sinT = glm::sqrt(1.0 - (cosT * cosT));

	if (prd)printf("sinT: %f\n", sinT);

	if (prd)printf("downstream radiance: %s\n", glm::to_string(downstreamRadiance).c_str());

	dvec3 numerator = downstreamRadiance * fresnel * geometry*sinT;
	double denom = 4 * glm::dot(wo, n) * glm::dot(h, n);
