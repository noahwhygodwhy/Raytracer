...
{
public:
	Biconvex(const dvec3 origin, dvec3 forward, double sphereRadius, double focalLength, dmat4(*movementFunction)(double currentTime));
	~Biconvex();
...
private:
	...
	dvec3 origin;
	dvec3 forward;
	AABB setupSpheres(const dvec3 origin, dvec3 forward, double sphereRadius, double focalLength);
};

Biconvex::Biconvex(const dvec3 origin, dvec3 forward, double sphereRadius, double focalLength, dmat4(*movementFunction)(double currentTime))
	: Shape(this->setupSpheres(origin, forward, sphereRadius, focalLength))
...
AABB Biconvex::setupSpheres(const dvec3 origin, dvec3 forward, double sphereRadius, double focalLength) {
	...
	this->d = solveLensForD(sphereRadius, focalLength, glass.ni);//TODO: this is broken
	this->d = 0.2;
	...
	dvec3 offset = (sphereRadius - (forward * d));

	dvec3 s1Origin = origin + (sphereRadius - (forward * d));
	dvec3 s2Origin = origin - (sphereRadius - (forward * d));
